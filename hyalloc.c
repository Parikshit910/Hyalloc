...
/* #define CHUNKSIZE (1024 * 1024) 1 mb */
void* exp_heap = NULL;   // the heap
void* imp_addr_arr[MAX_MMAP_CHUNKS];
int num_of_imp_mmap = 0;
int num_of_mmap = 1;
void* imp_heap = NULL;   // the heap
#define ALIGN(size) (((size) + 7) & ~7)

struct mmap_info mmap_info_arr[MAX_MMAP_CHUNKS];
Block* head;
Block* main_blk;
...
int init_exp(void){
    exp_heap = mmap(NULL, CHUNKSIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0); 
    ...
    head = main_blk;
    mmap_info_arr[0].pointer = exp_heap;
    mmap_info_arr[0].size  = CHUNKSIZE;
    mmap_info_arr[1].pointer = (void*)((char*)exp_heap + CHUNKSIZE);
    mmap_info_arr[1].size  = CHUNKSIZE;
    return 0;
}
...
int init_imp(void){
  imp_heap  = mmap(NULL, CHUNKSIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0); 
  imp_addr_arr[num_of_imp_mmap] = imp_heap;
  num_of_imp_mmap++;
  int block_num = 0;
...
void inc_exp_heap(size_t size){
  ...
  num_of_mmap = 1 +  num_of_mmap;
  int index = 2 * num_of_mmap - 2;
  mmap_info_arr[index].pointer = new_main_blk;
  mmap_info_arr[index].size = size+CHUNKSIZE;
  mmap_info_arr[index + 1].pointer = (void*)((char*)new_main_blk + CHUNKSIZE + size);
  mmap_info_arr[index+1].size = size+CHUNKSIZE;
  main_blk = (Block*)new_main_blk;
  main_blk->is_Free = true;
  main_blk->next = main_blk;
  main_blk->prev = main_blk;
  main_blk->size = size + CHUNKSIZE - sizeof(Block);
      ...
      if((curr1->size) >= (old_main_blk->size)){
        ...
        old_main_blk->prev = prev_blk;
        old_main_blk->next = curr1;
        curr1->prev = old_main_blk;
      }
      curr1 = curr1->next;
    } while (curr1 != head );
}

void inc_imp_heap(void** ptr) {
  void* new_chunk = mmap(NULL, CHUNKSIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  imp_addr_arr[num_of_imp_mmap] = new_chunk;
  num_of_imp_mmap++;
  if (new_chunk == MAP_FAILED) {
    perror("MAP FAILED");
    return; // Function is void, so just return
  }

  void* curr = new_chunk;
  int block_num = 0;

  // Slice the new chunk into blocks
  for(int i = 0; i < CHUNKSIZE - THRESHOLD; i += THRESHOLD) {
    void* next_block = (char*)new_chunk + (block_num + THRESHOLD);
    *(void**)curr = next_block; // Set the current block to point to the next
    curr = next_block;
    block_num += THRESHOLD;
  }

  // The last block points to NULL (or you could point it to the OLD small_free_head!)
  *(void**)curr = NULL; 

  // CRITICAL: Update the actual small_free_head variable in hymalloc
  *ptr = new_chunk; 
}
...
int check_range(Block* ptr){
  uintptr_t ptr_cmp = (uintptr_t)ptr; 
  int count = num_of_mmap;
  for (int i = 1; i <= count; i++)
  {
    int index = (2 * i) - 2;
    if (ptr_cmp > (uintptr_t)mmap_info_arr[index].pointer && ptr_cmp < (uintptr_t)mmap_info_arr[index+1].pointer)
    {
      return 1;
    }
  }
...
void coalesce_everything(){
    ...
    do
    {
      ...
      while (check_range(next_blk) && next_blk->is_Free && next_blk->next != next_blk)
      {
...
void* hymalloc(size_t size){
  ...
  {  
    ...
    while (ptr != head)
    {
    if (ptr->size<ptr->prev->size)
    {
    Block* next_ptr = ptr->next;
    while ((ptr->prev)->size > ptr->size)
    { Block* curr_next = ptr->next;
    Block* curr_prev = ptr->prev;
    Block* new_next = ptr->prev;
    Block* new_prev = curr_prev->prev;
...
int hydestroy(void){
  for (int i = 1; i <= num_of_mmap; i++)
  {
    int index = (2*i)-2;
    munmap(mmap_info_arr[index].pointer, mmap_info_arr[index].size);
  }
  for (int i = 0; i < num_of_imp_mmap; i++)
  {
